...
export class ForegroundFallbackManager {
  ...
  /** Sessions owned by BackgroundTaskManager; foreground fallback must ignore them. */
  ...
  async handleEvent(rawEvent: unknown): Promise<void> {
    ...
    if (event.type === 'session.created') {
      ...
      if (info?.id && info.parentID) {
        this.backgroundSessions.add(info.id);
        return;
      }
    }

    if (sessionID && this.backgroundSessions.has(sessionID)) {
  ...
  private getSessionID(event: {
    ...
    switch (event.type) {
      case 'message.updated':
        return (
          event.properties as { info?: { sessionID?: string } } | undefined
        )?.info?.sessionID;
      case 'session.error':
      case 'session.status':
      case 'subagent.session.created':
