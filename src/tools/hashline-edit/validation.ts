...
const LINE_REF_EXTRACT_PATTERN = HASH_CAPTURE_PATTERN;
...
export function parseLineRef(ref: string): LineRef {
  ...
  if (hashIdx > 0) {
    ...
    if (!/^\d+$/.test(prefix) && HASH_PATTERN.test(suffix)) {
      throw new Error(
...
export class HashlineMismatchError extends Error {
  ...
  static formatMessage(
    ...
    const output: string[] = [];
    output.push(
      `${mismatches.length} line${mismatches.length === 1 ? ' has' : 's have'} changed since last read. ` +
...
function suggestLineForHash(ref: string, lines: string[]): string | null {
  const hashMatch = ref.trim().match(HASH_SUFFIX_PATTERN);
