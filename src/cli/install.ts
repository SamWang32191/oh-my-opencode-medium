...
function printHeader(isUpdate: boolean): void {
  console.log();
  console.log(
    `${BOLD}oh-my-opencode-medium ${isUpdate ? 'Update' : 'Install'}${RESET}`,
  );
...
async function runInstall(config: InstallConfig): Promise<number> {
  ...
  printStep(
    step++,
    totalSteps,
    'Writing oh-my-opencode-medium configuration...',
  );
  if (config.dryRun) {
  ...
  console.log(
    `  ${BLUE}https://github.com/SamWang32191/oh-my-opencode-medium/blob/medium/docs/provider-configurations.md${RESET}`,
  );
  console.log();
