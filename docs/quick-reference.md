...
```bash
export OH_MY_OPENCODE_MEDIUM_PRESET=openai
opencode
```
...
You can customize which skills each agent is allowed to use in `~/.config/opencode/oh-my-opencode-medium.json` (or `.jsonc`).
...
You can configure MCP access in your plugin configuration file: `~/.config/opencode/oh-my-opencode-medium.json` (or `.jsonc`).
...
1. **Enable tmux integration** in `oh-my-opencode-medium.json` (or `.jsonc`):
...
| `~/.config/opencode/oh-my-opencode-medium.json` or `.jsonc` | Plugin settings (agents, tmux, MCPs) |
| `.opencode/oh-my-opencode-medium.json` or `.jsonc` | Project-local plugin overrides (optional) |
...
You can customize agent prompts by creating markdown files in `~/.config/opencode/oh-my-opencode-medium/`:

- With no preset, prompt files are loaded directly from this directory.
- With `preset` set (for example `test`), the plugin first checks `~/.config/opencode/oh-my-opencode-medium/{preset}/`, then falls back to the root prompt directory.
...
```
~/.config/opencode/oh-my-opencode-medium/
  ├── test/
...
**File Priority:**
1. `oh-my-opencode-medium.jsonc` (preferred if exists)
2. `oh-my-opencode-medium.json` (fallback)
...
### Plugin Config (`oh-my-opencode-medium.json` or `oh-my-opencode-medium.jsonc`)
